...
Core metrics for the LMC study:
  - Linear interpolation of state dicts
  - Loss barrier B(θ_A, θ_B)
  - Linear CKA (Kornblith et al., 2019)
  - CKA heatmap matrix (layer × layer)
"""
...
def interpolate_state_dicts(sd_a, sd_b, alpha):
    ...
    Handles both float parameters and integer BN num_batches_tracked.
    """
    sd_interp = {}
    for key in sd_a:
        if sd_a[key].dtype in (torch.long, torch.int32, torch.int64):
            sd_interp[key] = sd_a[key]  # e.g. num_batches_tracked
        else:
...
def compute_barrier(
    ...
    Compute the linear loss barrier B(θ_A, θ_B):
        ...
        B = max_{α ∈ [0,1]} L(α·θ_A + (1−α)·θ_B)
            − ½ (L(θ_A) + L(θ_B))

    Parameters
    ----------
    model_class : type — model constructor
    model_kwargs : dict — kwargs for model constructor
    sd_a, sd_b : dict — state dicts of the two models
    loader : DataLoader — evaluation data
    device : torch.device
    n_steps : int — number of α values to evaluate
    return_curve : bool — if True, also return the full loss/acc curve

    Returns
    -------
    barrier : float — the loss barrier value
    curve : dict (if return_curve) with 'alphas', 'losses', 'accs'
    """
    criterion = nn.CrossEntropyLoss()
...
def cka_linear(X, Y):
    ...
    Compute linear CKA between two representation matrices:
        ...
        CKA_lin(X, Y) = ‖Y^T X‖²_F / (‖X^T X‖_F · ‖Y^T Y‖_F)

    Parameters
    ----------
    X, Y : Tensor of shape (n_samples, n_features)
...
def compute_cka_matrix(model_a, model_b, loader, device, max_batches=5):
    ...
    Returns
    -------
    cka_mat : np.ndarray of shape (n_layers_a, n_layers_b)
    layer_names_a, layer_names_b : list[str]
    """
