...
Key functions
─────────────
  train_one_epoch   — single epoch of training
  evaluate          — compute loss & accuracy on a loader
  train_model       — full training loop with optional checkpointing
  train_spawned_pair — train two models from a shared start (spawning)
"""
...
def train_spawned_pair(
    ...
    This implements the *spawning* protocol from Frankle et al. (2020)
    used to test H1 (Connectivity Origin) with strict dataloader isolation.

    Returns
    -------
    model_a, model_b, history_a, history_b
    """
    from .utils import get_device  # 🛡️ CORRECCIÓN: Se elimina get_dataloaders redundante
    if device is None:
        device = get_device()
...
def run_multi_seed(experiment_fn, seeds, **kwargs):
    ...
    Parameters
    ----------
    experiment_fn : callable(seed, **kwargs) → dict[str, float]
        Must accept 'seed' as first arg and return a dict of scalar metrics.
    seeds : list[int]
        Seeds to run.
    **kwargs : forwarded to experiment_fn.

    Returns
    -------
    results : dict — each key maps to {'mean': float, 'std': float, 'values': list}
    """
    import numpy as np
