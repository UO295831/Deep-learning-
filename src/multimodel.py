...
def ties_merge(state_dicts, base_sd=None, k_fraction=0.2):
    ...
    1. Compute task vectors τ_i = θ_i - θ_base
    2. Trim: zero out the bottom (1-k) fraction of params by magnitude
    3. Elect sign: for each param, majority vote on sign
    4. Disjoint merge: average only params that agree with elected sign

    If base_sd is None, uses the mean as reference.
    """
    n = len(state_dicts)
    ...
    for key in base_sd:
        ...
        # 1. Trim: zero out smallest magnitudes
        ...
        # 3. Disjoint merge: average only agreeing params
...
def slerp_barycenter_merge(state_dicts, max_iter=50, tol=1e-6):
    ...
    Algorithm: iterative Fréchet mean
      1. Initialize μ = mean of normalized vectors
      2. For each iteration:
         - For each θ_i, compute tangent vector via log map
         - Update μ along the average tangent direction
...
def compute_multimodel_barrier(model_class, model_kwargs, merged_sd,
    ...
    """
    Compute B_n = L(θ_merged) - (1/n) Σ L(θ_i)
    """
