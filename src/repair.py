...
Implements the REPAIR algorithm from Jordan et al. (2023) and several
variants for correcting the variance collapse that occurs when
linearly interpolating neural network weights.

Key insight: if h_A and h_B are roughly independent with variance σ²,
then Var((h_A + h_B)/2) ≈ σ²/2 — the merged model's activations have
half the variance, which cascades through depth.

Variants
--------
  repair_full          — Full REPAIR: rescale all layers' activations
  repair_bn_recalibration — Only re-estimate BN running stats (very cheap)
  repair_reset_retrain_bn — Reset BN and retrain affine params (γ, β)
  repair_layerwise     — Apply REPAIR only to selected layers
"""
...
def variance_collapse_ratio(model_merged, model_a, model_b,
    ...
    Compute the per-layer variance collapse ratio:
        ...
        r^(l) = σ_merged^(l) / σ_expected^(l)

    where σ_expected = (σ_A + σ_B) / 2  (theoretical prediction ≈ 1/√2
    for the first layer, decreasing with depth).

    Returns
    -------
    ratios : dict[str, Tensor] — ratio per unit/channel per layer
    """
...
def repair_full(model_merged, model_a, model_b, alpha,
    ...
    Parameters
    ----------
    model_merged : nn.Module — the interpolated model (weights already set)
    model_a, model_b : nn.Module — the two parent models
    alpha : float — interpolation coefficient (α·θ_A + (1−α)·θ_B)
    calib_loader : DataLoader — calibration data (no augmentation)
    device : torch.device

    Returns
    -------
    model_repaired : nn.Module — model with rescaling hooks installed
    """
...
def repair_bn_recalibration(model_merged, calib_loader, device):
    """
    Re-estimate BatchNorm running statistics (μ, σ²) by forwarding
    calibration data through the merged model.

    This is the cheapest repair variant — no trainable parameters are
    modified, only BN running_mean and running_var are updated.

    Returns
    -------
    model_recalib : nn.Module (deep copy with recalibrated BN)
    """
...
def repair_reset_retrain_bn(model_merged, calib_loader, device,
                            lr=0.01, epochs=3):
    """
    Reset BN running stats, then retrain the BN affine parameters
    (weight=γ, bias=β) while keeping all other weights frozen.
...
def variance_collapse_vs_alpha(model_class, model_kwargs, sd_a, sd_b,
    ...
    Theory predicts r ≈ 1/√2 is maximal at α=0.5 and approaches 1.0
    at α→0 and α→1 (no mixing).

    Parameters
    ----------
    model_class, model_kwargs : model constructor
    sd_a, sd_b : aligned state dicts
    loader : calibration DataLoader
    device : torch.device
    alphas : list[float] — α values to sweep

    Returns
    -------
    results : dict[float, dict[str, Tensor]]
        α → {layer_name → ratio_tensor}
    """
