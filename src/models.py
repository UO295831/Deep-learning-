...
class MLP3(nn.Module):
    ...
    Architecture
    ────────────
    Flatten(784) → FC(784→h) → ReLU
                 → FC(h→h)   → ReLU
                 → FC(h→h)   → ReLU
                 → FC(h→C)

    No BatchNorm — pure permutation symmetry on hidden units.
    This is the cleanest setting for testing weight/activation matching.
    """
...
class SimpleConvBN(nn.Module):
    ...
    Architecture
    ────────────
    Conv2d(3→32, 3×3)   → BN2d → ReLU → MaxPool(2)    [32×16×16]
    Conv2d(32→64, 3×3)  → BN2d → ReLU → MaxPool(2)    [64×8×8]
    Conv2d(64→128, 3×3) → BN2d → ReLU → MaxPool(2)    [128×4×4]
    Flatten(2048) → FC(2048→256) → BN1d → ReLU
    FC(256→C)

    Has both BN2d (conv layers) and BN1d (FC layer), making it ideal
    for testing REPAIR's BN recalibration variants.
    """
...
class ConvMixer(nn.Module):
    ...
    Architecture
    ────────────
    PatchEmbed: Conv2d(3→256, p×p, stride=p) → GELU → BN
    8 × ConvMixerBlock(depthwise 5×5 → GELU → BN → pointwise 1×1 → GELU → BN)
    AdaptiveAvgPool2d(1) → FC(256→C)

    This is NOT a standard ConvNet, NOT a ViT. It demonstrates that
    simple patch + mixing operations suffice for reasonable performance.
    """
...
def get_model(name: str, **kwargs) -> nn.Module:
    ...
    >>> model = get_model("mlp3")
    >>> model = get_model("simpleconvbn", num_classes=10)
    >>> model = get_model("convmixer", num_classes=10)
    """
    registry = {
        "mlp3": MLP3,
