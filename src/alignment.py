...
Each method finds a transformation T such that the linear interpolation
  α·θ_A + (1−α)·T(θ_B)
has minimal loss barrier.
"""
...
def _get_mlp3_perm_spec():
    ...
    The i-th permutation P_i permutes the *output* dimension of layer i,
    and the *input* dimension of layer i+1.
    """
    return [
...
def _get_convbn_perm_spec():
    ...
    At the conv→fc boundary: permuting conv2 output channels (128)
    means permuting *blocks* of spatial_size² in fc0.weight columns.
    spatial_size = 4 (after 3x MaxPool2 on 32x32).
    """
    return [
...
def _collect_activations(model, loader, device, max_batches=5):
    ...
    # Register hooks on the weight-bearing modules
...
def activation_matching(model_a, model_b, loader, device):
    ...
    activations using LAP on the cross-correlation matrix:
        ...
        max_P  tr(H_A^T · H_B · P)

    Parameters
    ----------
    model_a, model_b : nn.Module
    loader : DataLoader (calibration data)
    device : torch.device
...
def procrustes_alignment(model_a, model_b, loader, device):
    ...
    to model_a via the Procrustes problem:
        ...
        min_{Q^T Q = I}  ‖H_A - H_B · Q‖_F

    Closed-form solution via SVD:
        H_B^T H_A = U Σ V^T  →  Q* = U V^T

    Note: orthogonal transforms do NOT commute with element-wise
    nonlinearities (ReLU), so this introduces an approximation error.
    That error is exactly what H2 measures.

    Returns
    -------
    transforms : list[torch.Tensor] — orthogonal matrices Q^(l)
    """
    acts_a = _collect_activations(model_a, loader, device)
    acts_b = _collect_activations(model_b, loader, device)
...
def apply_transform_to_state_dict(model_b, transforms):
    ...
    For each layer l:
      W_new^(l)   = Q^(l) · W^(l) · Q^(l-1)ᵀ  (approximately)
      b_new^(l)   = Q^(l) · b^(l)

    Returns a new state_dict. Does NOT modify model_b in-place.
    """
    spec = get_perm_spec(model_b)
    ...
    for layer_idx, Q in enumerate(transforms):
        ...
        if s.get("bn") is not None:
            bn_prefix = s["bn"]
            # FIX: Only rotate learnable vectors. Do NOT multiply running_mean 
            # or running_var by Q, as negative variances cause square-root NaNs.
...
def cycle_consistency_error(model_a, model_b, model_c, method="weight_matching",
    ...
    Given 3 models A, B, C, compute:
      T_{A→B}: alignment from A to B
      T_{B→C}: alignment from B to C
      T_{A→C}: alignment from A to C (direct)
      T_{A→C}': composition T_{B→C} ∘ T_{A→B}  (indirect)

    Error = ‖T_{A→C}(θ_C) − T_{A→C}'(θ_C)‖₂ / ‖θ_C‖₂

    For perfect cycle-consistency, this should be 0.

    Parameters
    ----------
    model_a, model_b, model_c : nn.Module
    method : 'weight_matching' or 'activation_matching'
    loader : DataLoader (needed for activation_matching)
    device : torch.device
